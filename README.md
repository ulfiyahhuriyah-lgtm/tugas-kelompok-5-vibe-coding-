...
> - [`docs/AI_USAGE.md`](docs/AI_USAGE.md) — bagaimana AI Coding Tool digunakan & bug yang ditemukan/diperbaiki
...
- **book-service** — pemilik data buku & transaksi peminjaman; **tidak pernah mempercayai `nim` yang dikirim langsung dari client**, melainkan memverifikasi token ke `user-service` lebih dulu (lihat [AI_USAGE.md](docs/AI_USAGE.md) untuk cerita bug ini).
...
aturan bisnis sudah dipindahkan ke sisi server. Jalankan `python3 tests/uji_verifikasi.py`
...
│   ├── AI_USAGE.md          # dokumentasi penggunaan AI Coding Tool + bug yang ditemukan
│   ├── PROMPTS.md           # seluruh prompt yang digunakan
│   └── GITHUB_SETUP.md      # panduan push & kontribusi anggota
├── tests/
│   ├── audit_ketentuan.py    # audit 9 ketentuan tugas terhadap isi repo
│   ├── uji_fitur_v2.py       # fitur baru: cari, saring, riwayat, sisa hari
│   ├── uji_antarmuka_v2.py   # keselarasan HTML/CSS/JS + keamanan
│   ├── uji_verifikasi.py     # regresi AC-01..04 + verifikasi bug tertutup
│   ├── uji_alur_frontend.py  # simulasi alur browser end-to-end
│   └── uji_adversarial.py    # skrip yang dipakai MENEMUKAN bug tersebut
├── user-service/
│   ├── server.js
│   ├── package.json
│   └── Dockerfile
...
| `ERR_CONNECTION_REFUSED` di browser | Tidak ada service yang berjalan di port tersebut | Jalankan servicenya |
...
curl -X POST http://localhost:4002/api/loans/LOAN_ID/return \
  -H "Content-Type: application/json" \
  -d '{"token":"TOKEN"}'
...
```bash
python3 tests/uji_verifikasi.py
# -> RINGKASAN: 21 LULUS, 0 GAGAL

python3 tests/uji_alur_frontend.py
# -> RINGKASAN ALUR BROWSER: 20 LULUS, 0 GAGAL

python3 tests/audit_ketentuan.py
# -> HASIL AUDIT: 41 OK, 0 GAGAL

python3 tests/uji_fitur_v2.py
# -> RINGKASAN FITUR v2: 25 LULUS, 0 GAGAL

python3 tests/uji_antarmuka_v2.py
...
| Skrip | Isi |
|---|---|
| `tests/audit_ketentuan.py` | Memeriksa kesembilan ketentuan tugas terhadap isi repo yang sebenarnya |
| `tests/uji_fitur_v2.py` | Fitur hasil pengembangan: pencarian, penyaringan, riwayat, sisa hari, otorisasi |
| `tests/uji_antarmuka_v2.py` | Keselarasan HTML, CSS, JavaScript, dan keamanan penyisipan data |
| `tests/uji_verifikasi.py` | Regresi AC-01 s/d AC-04 + verifikasi bug ronde 2 sudah tertutup |
| `tests/uji_alur_frontend.py` | Meniru urutan pemanggilan `app.js` di browser: muat aset → login → render → pinjam → kembalikan → sesi kedaluwarsa |
| `tests/uji_adversarial.py` | Skrip yang dipakai **menemukan** empat bug ronde 2 tersebut |
...
Skenario lengkap (termasuk hasil aktual saat pengembangan) ada di [`docs/AI_USAGE.md`](docs/AI_USAGE.md).
...
endpoint yang menerima `nim` mentah dari client — lihat [`AI_USAGE.md`](docs/AI_USAGE.md)
...
> Langkah teknisnya ada di [`docs/GITHUB_SETUP.md`](docs/GITHUB_SETUP.md).
